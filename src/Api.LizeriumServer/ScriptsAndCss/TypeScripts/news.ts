...
class NewsAdmin {
    ...
    bindSitemapRebuild() {
        ...
        form.addEventListener("submit", async (event) => {
            ...
            try {
                const response = await fetch(form.getAttribute("action") || "/sitemap/rebuild", {
                    method: "POST",
                ...
                if (!response.ok || !result || !result.ok) {
                    const message = result && (result.message || result.sitemapRebuild?.message)
    ...
    bindNewsImageFallback(image, rawUrl) {
        ...
        image.addEventListener("error", () => {
            ...
            image.closest(".news-icon-preview, .news-image-preview")?.classList.add("broken");
        });
    }
